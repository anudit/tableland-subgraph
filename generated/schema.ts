...
export class Chaindata extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Chaindata entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Chaindata must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("Chaindata", id.toString(), this);
    }
  }

  static load(id: string): Chaindata | null {
    return changetype<Chaindata | null>(store.get("Chaindata", id));
  }
  ...
  get tables(): BigInt {
    let value = this.get("tables");
    return value!.toBigInt();
  }

  set tables(value: BigInt) {
    this.set("tables", Value.fromBigInt(value));
  }
}
